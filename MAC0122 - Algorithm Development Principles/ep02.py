...
'''
    ...
    - LISTA de fontes externas utilizadas (links ou referências como livros)
        ...
        - Deste livro, foi retirada a função , a qual foi utilizada somente para que os resultados estivessem em sua forma irredutivel (não interferindo, de fato, nos resultados).
...
def HFmaior( n ):
    ''' 
    (int) -> (Fraction)
    Recebe um número inteiro (n) e retorna o número harmônico (de ordem n) calculando-o a partir do maior termo (1), da seguinte forma:
        1 +    1/2     +   1/3     + 1/4 + ... + 1/n 
    '''

    hmaior = Fraction()
    for i in range (n):
        hmaior += Fraction(1,i+1)
    menor_divisor_comum = mdc(hmaior.num, hmaior.den)
    final = Fraction(int((hmaior.num)/menor_divisor_comum),int((hmaior.den)/menor_divisor_comum))
    return final
...
def HFmenor( n ):
    ''' 
    (int) -> (Fraction)
    Recebe um número inteiro (n) e retorna o número harmônico (de ordem n) calculando-o a partir do menor termo (1/n), da seguinte forma:
        1/n +  1/(n-1) +   1/(n-2) + ... + 1/2 + 1
    '''
    hmenor = Fraction()
    for i in range (n,0,-1):
        hmenor += Fraction(1,i)
    menor_divisor_comum = mdc(hmenor.num, hmenor.den)
    final = Fraction(int((hmenor.num)/menor_divisor_comum),int((hmenor.den)/menor_divisor_comum))
    return final
