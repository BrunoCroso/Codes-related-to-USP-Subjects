...
'''
    ...
    Descrição de ajuda ou indicação de fonte:
        Este EP foi realizado sem ajuda externa
...
def main():
    ...
    for N in range(ini, fim, passo):
        ...
        print(f"q({N}) = {qN}")                                                                                 #'''Fica com mais uma tabulação ou não?''' 
        if mostre in ['s', 'S']: 
...
def experimento(N, T):
    ...
    for i in range (T):
        ...
        c = 0
        parada = False
        while c < len(lista_aleatoria) and parada == False:
            if lista_aleatoria[c] == c:
                lista_egoistas.append(lista_aleatoria)
                parada = True
            c += 1
    probabilidade = ((T-len(lista_egoistas))/T)
    return probabilidade,lista_egoistas
...
def altruistas(amigo_de):
    ...
    for i in range (len(amigo_de)):
        if amigo_de[i] == i and i < (len(amigo_de)-1):
            temporaria = amigo_de[i+1]
            amigo_de[i] = temporaria
            amigo_de[i+1] = i
        if amigo_de[i] == i and i == (len(amigo_de)-1):
            temporaria = amigo_de[0]
            amigo_de[i] = temporaria
            amigo_de[0] = i
