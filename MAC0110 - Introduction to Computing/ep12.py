...
def dicio_sucessores(lst):
    ...
    dicionario = {}
    if len(lst) != 0:
        dicionario[lst[len(lst)-1]] = [lst[0]]
        
        for i in range (len(lst)-1):
            if lst[i] in dicionario.keys():
                dicionario[lst[i]] += [lst[i+1]]
            elif lst[i] not in dicionario.keys():
                dicionario[lst[i]] = [lst[i+1]]
...
def gere_frase(p_inicial, dicio):
    ...
    if p_inicial in dicio.keys():
        elementos_da_frase.append(p_inicial)
        proximo = random.choice(dicio[p_inicial])
        while proximo not in PONTUACAO_FINAL:
            elementos_da_frase.append(proximo)
            proximo = random.choice(dicio[proximo])
        elementos_da_frase.append(proximo)
    
    for i in range (len(elementos_da_frase)):
        if elementos_da_frase[i] in PONTUACAO_SEM_ESPACO:
            frase += elementos_da_frase[i]
        else:
            frase += ' '
            frase += elementos_da_frase[i]
